					...
					<el-card v-for="item in filteredPromptList" :key="item.id" class="prompt-card" shadow="hover">
						...
						<div class="prompt-meta">
							...
							<div class="meta-item">
								...
								<span>Update time: {{ new Date(item.updated_at).toLocaleString() }}</span>
							</div>
						</div>
						...
						<div class="prompt-actions">
							<el-button class="btn-class" type="primary" link @click="handleEdit(item)">
...
const activeTab = ref(localStorage.getItem("promptActiveTab") || "system"); // Read cached tab from localStorage, default is system
...
// Computed property: whether to display pagination component (only displayed when data exceeds 10 items)
...
const getConfig = () => {
	getConfig_value("FUNCTION_PROMPT_TEMPLATE").then((res) => {
		...
		// Initialize data, prioritize using cached tab
...
const handleTabChange = (tab: string): void => {
	activeTab.value = tab;
	// Save current selected tab to localStorage
...
const handleDelete = (row: Prompt): void => {
	ElMessageBox.confirm("Are you sure you want to delete this prompt configuration?", "Tip", {
		type: "warning",
	}).then(async () => {
		try {
