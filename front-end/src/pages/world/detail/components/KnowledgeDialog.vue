<!-- KnowledgeDialog.vue - 添加/编辑知识对话框组件 -->
<script setup lang="ts">
	...
	const cascaderProps = ref({
		...
		emitPath: true, // Return complete path when selecting
		lazyLoad: lazyLoad,
	});
	/**
	...
	/**
	 * Watch cascader value changes and update related knowledge
	 */
	watch(cascaderValue, (newVal) => {
		// Since checkStrictly is false, only leaf nodes will be selected
		// Each selected item is an array like [worldId, knowledgeId]
		const knowledgeIds = newVal.map((path) => path[1]); // Get the second value of each path (knowledge ID)
	...
	/**
	 * Initialize from cascader selected values, knowledge association display issue
	 */
	const initCascaderValue = async () => {
		...
		cascaderProps.value.lazyLoad = async (node : any, resolve : (nodes : any[]) => void) => {
			...
			// Second level loads knowledge items of selected world
			else if (level === 1) {
				knowledgeLoading.value = true;
				try {
					const response = await getKnowledgeByWorld(value);
					if (response) {
						// Filter out the knowledge currently being edited (cannot associate with itself)
	 ...
	 */
	const enhancePrompt = ref(`Your task is to semantically enhance the following content for optimized vector database retrieval. Please follow these guidelines:

1. Content must be detailed and specific, avoiding excessive abstraction. Each block should be self-contained and complete to ensure relevant context is returned during vector database queries.
2. Use natural language expression: Present information in a coherent narrative style to enable vector embeddings to better capture semantic relationships.
...
Output requirements: Separate each block with two line breaks.`);
	...
	/**
	 * Model selection - using unified modelId
	 */
	...
	watch(
		...
		async (newVal) => {
			if (newVal) {
				...
				// If in edit mode, put text in the first text box
				if (props.dialogType === "edit") {
					...
					// Add mode, initialize with an empty text box
	...
	/**
	 * Watch prompt changes and save to localStorage
	 */
	...
	/**
	 * Watch model changes and save to localStorage
	 */
	...
	const handleEnhanceContent = () => {
		...
		enhanceContentTokenStream(
			...
			.then((response : any) => {
				// Request completed, check for remaining content in buffer
	...
	const submitKnowledge = async (formEl : FormInstance | undefined) => {
		if (!newKnowledge.value.title) {
			ElMessage.warning("Please fill in the title");
			return;
		}
	...
	// Load world list after component is mounted
	onMounted(() => {
		fetchWorldList();
	});
...
<template>
	<el-dialog :modelValue="visible" @update:model-value="handleDialogChange"
		:title="dialogType === 'add' ? 'Add Knowledge Fragment' : 'Edit Knowledge Fragment'" width="80%">
				...
				<el-form :model="newKnowledge" label-width="100px" :rules="formRules" ref="ruleFormRef">
					<FormItem label="Type" tooltipKey="type">
						<el-select v-model="newKnowledge.type" placeholder="Please select type">
							<el-option v-for="select in selectKnowledgeType" :key="select.value" :label="select.label"
								:value="select.value" />
						</el-select>
					</FormItem>
					...
					<FormItem label="Content" tooltipKey="knowledgeTextList[index]">
						<div v-for="(text, index) in knowledgeTextList" :key="index" class="text-item-container">
							<AvatarEditor :users="rolePrompt" :max-length="1000" :min-length="10" min-height="100px"
								placeholder="Please enter knowledge content..." v-model="knowledgeTextList[index]" />
							<div class="text-item-actions" v-if="dialogType === 'add'">
								<el-button type="danger" circle size="small" @click="removeTextItem(index)"
						...
						<div class="add-text-button" v-if="dialogType === 'add'">
							<el-button class="btn-fix" type="primary" plain size="small" @click="addTextItem">
								<el-icon>
									<Plus />
								</el-icon>
								Add Content Item
					...
					<FormItem label="Related Knowledge" tooltipKey="cascaderValue">
						<el-cascader v-model="cascaderValue" placeholder="Please select related knowledge" clearable
							:props="cascaderProps" filterable :loading="knowledgeLoading || worldLoading"
							style="width: 100%">
								...
								<span v-if="node.level === 1">
									<span class="knowledge-cascader-node">
										...
										<span class="knowledge-cascader-info" v-if="data.data">
											<el-tag size="small"
												:type="data.data.type === 'scene' ? 'success' : 'warning'">
												{{ data.data.type }}
											</el-tag>
					...
					<FormItem label="Related Characters" tooltipKey="relations_roles">
						<el-select v-model="newKnowledge.relations_roles" multiple filterable remote reserve-keyword
							placeholder="Please enter role name to search" :remote-method="searchRoles"
							:loading="allRolesLoading" style="width: 100%">
							<el-option v-for="role in allRolesList" :key="role.id"
								:label="`${role.name}(${role.role_id})`" :value="role.role_id">
			...
			<div class="knowledge-enhance-container" v-if="dialogType === 'add'">
				...
				<el-form>
					<FormItem label="Prompt" tooltipKey="enhancePrompt">
						<el-input v-model="enhancePrompt" type="textarea" :rows="3"
							placeholder="Please enter AI enhancement prompt" />
						<div class="prompt-hint">Prompt will be saved in local browser</div>
					</FormItem>

					<FormItem label="AI Model" tooltipKey="selectedModelId">
						<ModelCascader v-model="selectedModelId" />
					</FormItem>

					<el-form-item>
						<el-button type="primary" @click="handleEnhanceContent" :loading="isEnhancing"
							:disabled="!knowledgeTextList.some((text) => text.trim() !== '')" class="btn-fix">
							<el-icon>
								<Magic />
							</el-icon>
							Enhance Content
				...
				<div class="enhanced-results" v-if="enhancedResults.length > 0">
					...
					<el-scrollbar height="200px">
						<div class="results-container">
							<div v-for="(result, index) in enhancedResults" :key="index" class="enhanced-result-item"
					...
					<div class="result-hint">Click any result to apply to the left content box</div>
				</div>
			</div>
		</div>
		...
		<template #footer>
			<span class="dialog-footer">
				...
				<el-button class="btn-fix" type="primary" @click="submitKnowledge(ruleFormRef)">Confirm</el-button>
			</span>
		</template>
	</el-dialog>
</template>
