...
const addMessage = (role, content, roleName = null, data: any = {}) => {
	...
	if (!senderName) {
		switch (role) {
			case "human":
				senderName = taskSettings.userInfo.name || "你";
				break;
			case "setter":
				senderName = "出题者";
				break;
	...
	chatState.messages = [
		{
			...
			sender: "system",
			senderName: "系统",
			content: `欢迎来到${sourceType.value === "gamePlayType" ? "游戏" : "任务"}: ${taskSettings.taskInfo.title}\n\n${
...
const switchRole = (roleId) => {
	...
	if (selectedRole) {
		taskSettings.aiInfo.name = selectedRole.role_name || "AI助手";
		taskSettings.aiInfo.role = selectedRole.character_level ? `${selectedRole.character_level}级角色` : "任务辅助者";
	...
	if (chatState.isStarted) {
		chatState.messages.push({
			...
			sender: "system",
			senderName: "系统",
			content: `你现在正在与${taskSettings.aiInfo.name}对话`,
			timestamp: Date.now(),
		});
	}
};
...
const goBack = () => {
	if (chatState.isStarted) {
		ElMessageBox.confirm("确定要退出当前任务吗？进度将不会保存。", "提示", {
	...
	if (wsClient.value.isConnected()) {
		if (chatState.isAiTyping) {
			...
			return chatState.maxRounds === -1
				? `轮到你发言了 (当前第${chatState.currentRound}轮)`
				: `轮到你发言了 (${chatState.currentRound}/${chatState.maxRounds})`;
		}
	} else {
		return "正在连接服务器...";
	}
