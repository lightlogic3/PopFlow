		...
		<div class="editor-layout">
			...
			<div class="preview-panel">
				...
				<div v-else class="preview-placeholder">
					<el-empty description="Preview is hidden" />
				</div>
			</div>
		</div>
	</div>
...
// Watch props schema, update local data
watch(
	() => props.schema,
	(newVal) => {
...
// Watch props UI Schema, update local data
watch(
	() => props.uiSchema,
	(newVal) => {
		// Only update local state when value actually changes
...
// Watch props validation Schema, update local data
watch(
...
watch(
	() => localSchema.value,
	(newVal) => {
		// Only trigger events when value actually changes and not during update process
...
function onJsonChange(type: string) {
	try {
		// Try to parse JSON to check if format is correct
...
function applyJsonChanges(type: string) {
	try {
		switch (type) {
			...
			case "validationSchema":
				if (validationSchemaJson.value) {
					...
					// If validation rules found at root level, move them to rules object
		...
		ElMessage.error(`Applying JSON changes failed: ${error.message}`);
	}
}
...
function syncPreview() {
	try {
		console.log("Starting sync preview...");
		// Deep copy schema
		...
		// Update field required property based on validation rules
			...
			schemaClone.components.forEach((section) => {
				if (section.fields && Array.isArray(section.fields)) {
					section.fields.forEach((field) => {
						...
						// Check if field has required validation rule
		...
		// Increase key to force re-render
		...
		ElMessage.error(`Sync preview failed: ${error instanceof Error ? error.message : String(error)}`);
	}
}

// Ensure initialized UI Schema when component is mounted
onMounted(() => {
	...
	// Ensure local state is correctly initialized
