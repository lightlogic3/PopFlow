<template>
	<div class="game-types-container">
		...
		<div class="game-types-grid" v-loading="loading">
			<el-empty v-if="filteredGameTypes.length === 0" description="No game types available" class="empty-game-types" />
			...
			>
				...
				<div class="game-type-description">
					{{ gameType.description || "No description available" }}
				</div>

				<div class="game-type-details">
					<div class="detail-item" v-if="gameType.player_count">
						<el-icon><User /></el-icon>
						<span>Players: {{ gameType.player_count }}</span>
					</div>
...
const loadGameTypes = async () => {
	...
	try {
		...
		// Check if there's data, if not use mock data
...
const filterGameTypes = () => {
	// Filtering logic handled automatically by computed property
};
...
const editGameType = (gameType) => {
	// Mock data doesn't support editing, show notification
	if (gameType.id.toString().startsWith("mock-")) {
		ElMessage.warning("This is mock data, editing operation is not supported");
		return;
	}
...
const deleteGameType = (gameType) => {
	// Mock data doesn't support deletion, show notification
	if (gameType.id.toString().startsWith("mock-")) {
		ElMessage.warning("This is mock data, delete operation is not supported");
		return;
	}

	ElMessageBox.confirm(`Are you sure you want to delete the game type "${gameType.name}"? This action cannot be undone.`, "Delete Confirmation", {
		...
		.then(async () => {
			try {
				...
				ElMessage.success("Game type has been deleted");
