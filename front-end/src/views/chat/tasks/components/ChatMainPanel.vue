<template>
	<div class="chat-main">
		<div class="chat-header">
			...
			<div class="chat-rounds">Conversation rounds: {{ maxRounds === -1 ? "Unlimited" : `${currentRound}/${maxRounds}` }}</div>
		</div>

		<div class="chat-content" ref="chatContentRef">
			<div v-for="message in messages" :key="message.id" :class="['message-item', `message-${message.sender}`]">
				<div class="message-avatar">
					<img v-if="message.imageUrl" :src="message.imageUrl" alt="Character avatar" />
		...
		>
			<div v-if="isAiTyping" class="waiting-overlay">AI is thinking...</div>
			<el-input
				ref="inputRef"
				v-model="userInput"
				type="textarea"
...
/**
 * Chat Main Panel Component
 * @description Displays message list and input box
 */
...
interface ChatMessage {
	...
	content: string;
	timestamp: number;
	imageUrl?: string; // Optional avatar URL field
}
...
const scrollToBottom = async () => {
	if (chatContentRef.value) {
		...
		// Double insurance: add delay to scroll to bottom again, preventing inaccurate height calculation due to dynamic content
		setTimeout(() => {
			chatContent.scrollTop = chatContent.scrollHeight;
		}, 50);
	}
...
// Use computed property to achieve two-way binding
...
const handleSendMessage = () => {
	...
	// 1. Has content and not blank
	// 2. Not in loading state
	// 3. AI is not in typing state
	// 4. Rounds haven't reached the limit (if not unlimited rounds mode)
	if (
...
// Watch message changes, automatically scroll to bottom
watch(
...
// Watch AI typing state changes
watch(
	() => props.isAiTyping,
	(newValue) => {
		if (!newValue) {
			// AI stopped typing, user can input now
			focusInput();
		}
		scrollToBottom();
	},
...
const focusInput = async () => {
	if (inputRef.value && !props.isLoading && !props.isAiTyping) {
		await nextTick();
		setTimeout(() => {
			// Get the actual input element and focus
