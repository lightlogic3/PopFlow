	...
	<div class="field-editor">
		<el-form :model="localField" label-width="120px">
			...
			<el-form-item label="Display Label" required>
				<el-input v-model="localField.label" placeholder="Enter label displayed in form (e.g.: Wolf Count)" />
			</el-form-item>
			...
			<el-form-item label="Field Description">
				<el-input v-model="localField.placeholder" placeholder="Enter field hint information" />
			</el-form-item>
			...
			<template v-if="localField.type === 'switch'">
				...
				<el-form-item label="Active Text">
					<el-input v-model="localField.activeText" placeholder="e.g.: Yes" />
				</el-form-item>
				<el-form-item label="Inactive Text">
			...
			<template v-if="localField.type === 'dynamic-tags'">
				<el-form-item label="Default Tags">
					<el-select
						...
						placeholder="Enter and press Enter to add default tags"
						/>
					</el-form-item>
				</template>
			...
			<el-collapse>
				<el-collapse-item title="Advanced Properties" name="advanced">
					...
					<el-form-item label="CSS Class Name">
...
function validateName(value) {
	...
	if (!namePattern.test(value)) {
		nameError.value = "Invalid field name format, recommend using standard naming, e.g.: roles.wolf, rules.firstNightKill";
		return;
	}

	// Check if name conflicts with existing fields
...
function handleTypeChange(type) {
	...
	if (type === "text") {
		...
		// Provide more suitable default value range for game rule type fields
		...
		// Provide more suitable default range for game settings type fields
		...
		if (!localField.value.default) localField.value.default = [];
		if (!localField.value.placeholder) localField.value.placeholder = "Enter and press Enter to add";
	} else if (type === "color") {
