...
const handleDelete = (worldId: string) => {
	ElMessageBox.confirm("Are you sure you want to delete this world? This operation cannot be recovered", "Delete Confirmation", {
			...
			<div class="header-content">
				<div class="title-area">
					...
					<span class="page-subtitle"
						>Total {{ total }} worlds, {{ worlds.reduce((sum, w) => sum + w.knowledgeCount, 0) }} knowledge entries</span
					>
				</div>
				<el-button type="primary" @click="addWorld" size="large">
		...
		<div class="world-bookshelf" v-loading="loading">
			...
			<div class="books-container">
				<div
					...
					:key="world.id"
					class="world-tome"
					:class="`tome-${(index % 4) + 1}`"
					@click="goToDetail(world.id)"
				>
					...
					<div class="tome-cover">
						...
						<div class="tome-actions">
							<el-button
								size="small"
								circle
								@click.stop="goToDetail(world.id)"
								class="action-btn view-btn"
								title="View Details"
							>
								<el-icon><View /></el-icon>
							</el-button>
							<el-button size="small" circle @click.stop="handleEdit(world)" class="action-btn edit-btn" title="Edit">
								<el-icon><Edit /></el-icon>
							</el-button>
							<el-button
								size="small"
								circle
								@click.stop="handleDelete(world.id)"
		...
		<el-dialog v-model="dialogVisible" :title="dialogTitle" width="600px" class="world-dialog">
			...
			>
				<el-form-item label="World Title" prop="title">
					<el-input v-model="form.title" placeholder="Give your world a resonant name" />
				</el-form-item>
				<el-form-item label="World Type" prop="type">
					<el-select v-model="form.type" placeholder="Select world type">
						<el-option v-for="item in worldTypeOptions" :key="item.value" :label="item.label" :value="item.value" />
					</el-select>
				</el-form-item>
				<el-form-item label="Importance" prop="sort">
				...
				<el-form-item label="World Description" prop="description">
					<el-input
						...
						type="textarea"
						:rows="4"
						placeholder="Describe this world's background, features, and main content..."
					/>
				</el-form-item>
			...
			<template #footer>
				...
				<el-button type="primary" @click="submitForm(worldFormRef)" :loading="formLoading" size="large">
