...
// Add binding role related states
...
// Multiple text inputs - frontend temporary use
const knowledgeTextList = ref<string[]>([""]);

// Add AI enhancement related data
const enhancePrompt = ref(`Your task is to semantically enhance the following content for better vector database retrieval. Please follow these guidelines:

1. Content should be detailed and specific, avoiding abstraction. Each block should be independent and complete, making it easier to return relevant context when retrieving from the vector database.
...
// Get locally stored prompts in onBeforeMount
onBeforeMount(() => {
	...
	if (savedPlatform && platforms.value.some((p) => p.value === savedPlatform)) {
		...
		if (savedModel && models.value[selectedPlatform.value].some((m) => m.value === savedModel)) {
			...
			// If no corresponding model is found or not saved, set to the first model of the current platform
...
// Watch for prompt changes, save to localStorage
watch(enhancePrompt, (newVal) => {
	localStorage.setItem("aiEnhancePrompt", newVal);
});

// Watch for platform changes, update model selection and save to localStorage
watch(selectedPlatform, (newVal) => {
	// If the currently selected model is not in the model list of the new platform, reset to the first model of the new platform
...
// Watch for model changes, save to localStorage
...
const fetchKnowledge = async () => {
	...
	try {
		...
		ElMessage.error("Failed to get knowledge list");
		knowledgeList.value = [];
		knowledgeTotal.value = 0;
	} finally {
...
// Get related roles list
const fetchRoles = async () => {
	rolesLoading.value = true;
	try {
		// Decide which API to get based on current state
		let response;
		if (isShowingKnowledgeRoles.value && currentKnowledgeId.value) {
			// Get roles associated with specific knowledge point
			...
			// Get roles associated with the world
		...
		if (response) {
			...
			console.log("No related roles found");
			rolesList.value = [];
			rolesTotal.value = 0;
		}
	} catch (error) {
		console.error("Failed to get related roles", error);
		ElMessage.error("Failed to get related roles");
		rolesList.value = [];
		rolesTotal.value = 0;
	} finally {
...
// Get all roles list (for binding)
const fetchAllRoles = async () => {
	allRolesLoading.value = true;
	try {
		// Use role API to get all roles
		...
		ElMessage.error("Failed to get all roles");
		allRolesList.value = [];
		allRolesTotal.value = 0;
	} finally {
...
// Bind roles to currently selected knowledge point
const bindRolesToKnowledge = async (roleIds: string[]) => {
	...
	try {
		...
		// Create association for each selected role
		...
		ElMessage.success("Roles bound successfully");
		bindRoleDialogVisible.value = false;

		// Refresh related roles list
		fetchRoles();
	} catch (error) {
		console.error("Failed to bind role", error);
		ElMessage.error("Failed to bind role, role may already be bound");
	}
};

// Unbind role from knowledge point
const unbindRoleFromKnowledge = async (relationId: number) => {
	...
	try {
		await ElMessageBox.confirm("Are you sure you want to unbind this role?", "Tip", {
		...
		if (error !== "cancel") {
			...
			ElMessage.error("Failed to unbind");
		}
	}
};

// Watch all roles pagination change
const handleAllRolesPageChange = (page: number) => {
	allRolesCurrentPage.value = page;
	fetchAllRoles();
};

// Get roles related to knowledge point
...
// Reset to show roles related to the world
...
const fetchWorld = async () => {
	...
	try {
		...
		if (response) {
			// Correctly parse nested data field
		...
		ElMessage.error("Failed to get world details");
	} finally {
		loading.value = false;
	}
};
...
const editKnowledge = (knowledge: WorldKnowledge) => {
	...
	// Put existing content in first text box
...
const handleKnowledgeSubmit = async (knowledge: WorldKnowledgeCreate, validTexts: string[]) => {
	try {
		...
		if (knowledgeDialogType.value === "add") {
			// Add mode: add each content as a separate knowledge item
			const baseTitle = knowledge.title;
			// If there is only one content, use the original title directly
			if (validTexts.length === 1) {
				baseData["text"] = validTexts[0];
				baseData["title"] = baseTitle;
				await createWorldKnowledge(baseData);
			} else {
				// Multiple contents, create separate knowledge items for each content, add sequence number to title
			...
			// Edit mode: only modify the currently selected knowledge item
			baseData["text"] = validTexts[0];
...
// Handle knowledge pagination change
