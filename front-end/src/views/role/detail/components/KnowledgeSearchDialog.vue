<template>
	<el-dialog v-model="dialogVisible" title="Knowledge Search" width="80%" draggable>
		<div class="search-container">
					...
					<el-col :span="12">
						<el-form-item label="Similarity Threshold">
							<el-input-number
								...
								:step="0.01"
								:precision="2"
								placeholder="Score threshold (lower is more similar)"
							/>
							<div class="threshold-hint">Results with scores higher than this will be filtered</div>
						</el-form-item>
					</el-col>
				</el-row>
				<el-form-item>
			...
			<div class="search-results" v-if="filteredResults.length > 0">
				...
				<el-tabs v-model="activeTab" class="results-tabs">
					...
					<el-tab-pane label="World Settings" name="world">
						<div class="results-list">
							<el-empty v-if="filteredWorldResults.length === 0" description="No world setting results" />
							<el-card
			...
			<div v-else-if="searchResults.length > 0" class="no-results">
				<el-empty description="All results were filtered by threshold, please adjust the threshold setting" />
			</div>
		</div>
...
const emit = defineEmits<{
	...
	(e: "select-knowledge", knowledge: any): void;
}>();
...
const handleSearch = async () => {
	...
	try {
		...
		// Request from both APIs simultaneously
		...
		// Sort by similarity (lower score means more similar, so ascending order)
 ...
 */
const selectKnowledge = (result: any) => {
	emit("select-knowledge", result);
	ElMessage.success("Knowledge selected successfully");
};
</script>
