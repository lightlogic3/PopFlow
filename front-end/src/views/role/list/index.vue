...
// Add currently selected role id
...
const fetchRoles = async () => {
	...
	try {
		...
		if (response && response.items) {
			// Format data to adapt to frontend display
			...
			ElMessage.warning("No role data obtained");
			roles.value = [];
			total.value = 0;
		}
	} catch (error) {
...
const fetchProviders = async () => {
	try {
		...
		ElMessage.error("Failed to get LLM provider list, showing simulated data");
	}
};
function getConfig() {
...
function handleModelChange(modelInfo: LLMModel & { provider_name: string; provider_id: number }) {
	...
	// For example, update the interface display or save additional model information
}
...
<template>
	<div class="role-list-container">
		<div class="role-list-header">
			...
			<el-button type="primary" @click="addRole" class="add-role-btn">
								...
								<div class="role-stats">
									<div>
										<el-tag size="small" type="warning" style="margin-right: 5px">
											{{ role.llm_choose || "Doubao Role Play" }}
										</el-tag>
										<el-tag
		...
		<el-dialog v-model="dialogVisible" :title="dialogTitle" width="500px">
			...
			>
				...
				<el-collapse>
					<el-collapse-item title="Advanced Settings" name="1">
						...
						<FormItem label="Dialog Large Model" prop="llm_choose" style="width: 100%" tooltipKey="llm_choose">
			...
			<template #footer>
				...
				<el-button type="primary" @click="submitForm(roleFormRef)" :loading="formLoading"> Confirm </el-button>
			</template>
		</el-dialog>
	</div>
</template>
