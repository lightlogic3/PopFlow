<template>
	<div class="role-subtasks">
		<div class="header fade-in">
			...
			<div class="actions">
				<el-button type="primary" disabled @click="handleAddTask">
					<el-icon>
						<Plus/>
					</el-icon>
					Add Main Task
		...
		<div class="table-container fade-in-up">
			...
			>
				...
				<el-table-column type="expand">
					<template #default="props">
						<div class="subtasks-container">
							...
							>
								<el-table-column prop="task_personality" label="Subtask Character" show-overflow-tooltip/>
								<el-table-column prop="task_goal_judge" label="Judge Criteria" show-overflow-tooltip/>
								...
								<el-table-column prop="prologues" label="Opening" show-overflow-tooltip/>
				...
				<el-table-column prop="task_level" label="Difficulty Level" width="100"/>
				<el-table-column prop="task_description" label="Difficulty Level" width="100"/>
		...
		<el-dialog v-model="taskDialogVisible" :title="taskFormMode === 'add' ? 'Add Main Task' : 'Edit Main Task'"
							 width="800px">
			<el-form :model="taskForm" label-width="120px" ref="taskFormRef" :rules="taskFormRules">
				...
				<el-form-item label="Judge Criteria" prop="task_goal_judge">
					<el-input v-model="taskForm.task_goal_judge" type="textarea" placeholder="Please enter judge criteria"/>
				</el-form-item>
				<el-form-item label="Opening" prop="prologues">
					<el-input v-model="taskForm.prologues" type="textarea"
										placeholder="Please enter opening lines, separate multiple with commas"/>
				</el-form-item>
				<el-form-item label="Max Dialogue Rounds" prop="max_rounds">
				...
				<el-form-item label="Character Setting" prop="task_personality">
					<el-input v-model="taskForm.task_personality" type="textarea" placeholder="Please enter character setting"/>
				</el-form-item>
				<el-form-item label="Hidden Settings" prop="hide_designs">
					<el-input v-model="taskForm.hide_designs" type="textarea"
										placeholder="Please enter hidden settings, separate multiple with commas"/>
				</el-form-item>
		...
		<el-dialog
			...
			:title="subtaskFormMode === 'add' ? 'Add Subtask' : 'Edit Subtask1'"
			width="800px"
		>
			<el-form :model="subtaskForm" label-width="120px" ref="subtaskFormRef" :rules="subtaskFormRules">
				...
				<el-form-item label="Subtask Character" prop="task_personality">
					<el-input v-model="subtaskForm.task_personality" type="textarea"
										placeholder="Please enter subtask character"/>
				</el-form-item>
				<el-form-item label="Judge Criteria" prop="task_goal_judge">
					<el-input v-model="subtaskForm.task_goal_judge" type="textarea" placeholder="Please enter judge criteria"/>
				</el-form-item>
				<el-form-item label="Hidden Settings" prop="hide_designs">
					<el-input v-model="subtaskForm.hide_designs" type="textarea"
										placeholder="Please enter hidden settings, separate multiple with commas"/>
				</el-form-item>
				<el-form-item label="Difficulty Level" prop="task_level">
					<el-rate v-model="subtaskForm.task_level" :max="5"/>
				</el-form-item>
				<el-form-item label="description" prop="task_level">
					<el-input type="textarea" v-model="subtaskForm.task_description" :rows="10"></el-input>
				</el-form-item>
				<el-form-item label="task goal" prop="task_goal">
					<el-input type="textarea" v-model="subtaskForm.task_goal" :rows="10"></el-input>
				</el-form-item>
				<el-form-item label="Opening" prop="prologues">
					<el-input v-model="subtaskForm.prologues" type="textarea"
										placeholder="Please enter opening lines, separate multiple with commas"/>
				</el-form-item>
			</el-form>
			<template #footer>
		...
		<el-dialog v-model="autoTaskDialogVisible" title="Auto Generate Task" width="500px">
			<el-form :model="autoTaskForm" label-width="120px" ref="autoTaskFormRef" :rules="autoTaskFormRules">
				...
				<el-form-item label="Number of Tasks" prop="number">
...
const autoTaskFormRules = reactive<FormRules>({
	...
	number: [{required: true, message: "Task number cannot be empty", trigger: "blur"}],
	daily_time: [
		{
			required: true,
...
const loadSubtasks = async (taskId: string, page = 1, size = 10) => {
	// Initialize or update subtask pagination info
...
const handleDeleteTask = (row: RoleTask) => {
	ElMessageBox.confirm("Are you sure you want to delete this task? This will also delete all subtasks!", "Prompt", {
...
const handleDeleteSubtask = (row: RoleSubtask) => {
	ElMessageBox.confirm("Are you sure you want to delete this subtask?", "Prompt", {
		confirmButtonText: "Confirm",
		cancelButtonText: "Cancel",
		type: "warning",
	})
	...
	await autoTaskFormRef.value.validate(async (valid) => {
		if (valid) {
			try {
				...
				// If it's a scheduled task, ensure correct time is passed
				if (formData.regular_time && formData.daily_time) {
					// Keep daily_time format unchanged to ensure hour and minute information is included
					// Backend will extract hour and minute
				}

				await SubtaskApi.autoGenerateTask(formData);
				ElMessage.success("Auto task submitted, please manually refresh to view results");
				autoTaskDialogVisible.value = false;

				ElMessageBox.confirm("Do you want to go check task execution plan?", "Warning", {
...
// Handle random task completed
const handleRandomTaskCompleted = (result) => {
	console.log("Random task completed", result);
	// Can perform some operations, such as saving record, displaying result, etc.
};
...
const handleRandomTaskSkipped = () => {
	...
	ElMessage({
		type: "info",
		message: "Current random task skipped",
	});
};
</script>
