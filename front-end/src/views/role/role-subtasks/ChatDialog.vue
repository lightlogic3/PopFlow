			...
			<el-col :span="18" class="chat-right-panel">
				...
				<div class="chat-input">
					<el-input
						...
						type="textarea"
						:rows="3"
						:placeholder="isLoading ? 'Waiting for response...' : sessionInitialized ? 'Enter your message' : 'Please initialize the session first'"
...
const dialogTitle = computed(() => `Random Task Conversation: ${taskInfo.value.title || "Start New Task"}`);
...
// Watch dialog status changes and notify parent component
...
// Watch messages changes and auto scroll
watch(
	chatMessages,
	() => {
		nextTick(() => {
...
function addSystemMessage() {
	// Add opening or task explanation
	const intro = taskInfo.value.prologues || `Welcome to the "${taskInfo.value.title}" task. Goal: ${taskInfo.value.task_goal}`;
...
async function sendMessage() {
	...
	try {
		...
		const requestData = {
			...
			// The following parameters are retained as required by the interface, but are no longer used
			taskDescription: "",
			taskGoal: "",
			scoreRange: "",
			maxRounds: 10,
		...
		// Check and process tool_results
		if (response.data.tool_results) {
			let toolResults = response.data.tool_results;

			// If tool_results is a string, parse it to object
		...
		chatMessages.value.push({
			fromUser: false,
			content: response.data.message || "Sorry, I didn't receive a response.",
			time: new Date().toLocaleTimeString(),
			scoreChange,
			scoreReason,
		});
...
async function skipCurrentTask() {
	if (!sessionInitialized.value || !userInfo.value.user_id || !subTaskId.value) {
		ElMessage.warning("Cannot skip task, please make sure the session is initialized and user ID is provided");
		return;
	}

	// Confirm whether to skip
	try {
		await ElMessageBox.confirm("Are you sure you want to skip the current task?", "Prompt", {
		...
		ElMessage.success("Successfully skipped the current task");
