...
from plugIns.memory_system.mem0_memory.mem0.configs.base import MemoryConfig, MemoryItem
from plugIns.memory_system.mem0_memory.mem0.configs.enums import MemoryType
from plugIns.memory_system.mem0_memory.mem0.configs.prompts import (
    PROCEDURAL_MEMORY_SYSTEM_PROMPT,
    get_update_memory_messages,
)
from plugIns.memory_system.mem0_memory.mem0.memory.base import MemoryBase
from plugIns.memory_system.mem0_memory.mem0.memory.setup import mem0_dir, setup_config
from plugIns.memory_system.mem0_memory.mem0.memory.storage import SQLiteManager
from plugIns.memory_system.mem0_memory.mem0.memory.telemetry import capture_event
from plugIns.memory_system.mem0_memory.mem0.memory.utils import (
    get_fact_retrieval_messages,
    parse_messages,
    parse_vision_messages,
    remove_code_blocks,
)
from plugIns.memory_system.mem0_memory.mem0.utils.factory import EmbedderFactory, LlmFactory, VectorStoreFactory
...
class Memory(MemoryBase):
    def __init__(self, config: MemoryConfig = MemoryConfig()):
        ...
        if self.config.graph_store.config:
            if self.config.graph_store.provider == "memgraph":
                from plugIns.memory_system.mem0_memory.mem0.memory.memgraph_memory import MemoryGraph
            else:
                from plugIns.memory_system.mem0_memory.mem0.memory.graph_memory import MemoryGraph
    ...
    def get(self, memory_id):
        ...
        memory = asyncio.get_event_loop().run_until_complete(self.vector_store.get(vector_id=memory_id))
        if not memory:
            return None
    ...
    def _get_all_from_vector_store(self, filters, limit):
        memories_result = asyncio.get_event_loop().run_until_complete(self.vector_store.list(filters=filters, limit=limit))
    ...
    async def _search_vector_store(self, query, filters, limit):
        ...
        try:
            if asyncio.iscoroutinefunction(self.vector_store.search):
                ...
                # 检查返回值是否为协程对象
                if hasattr(memories, '__await__'):
                    logging.warning("search 返回了协程对象，自动等待结果")
                    memories = await memories
        except Exception as e:
            logging.error(f"搜索向量存储失败: {e}")
            import traceback
            traceback.print_exc()
            memories = []
        ...
        for memory in memories:
            # 如果 memory 为 None，跳过处理
            if memory is None:
                continue
                
            result_item = {}
            result_item["id"] = memory.id
            result_item["memory"] = memory.payload.get("data", "")
            result_item["score"] = memory.score if hasattr(memory, "score") else 0.0
            result_item["hash"] = memory.payload.get("hash")
            result_item["created_at"] = memory.payload.get("created_at")
            result_item["updated_at"] = memory.payload.get("updated_at")
    ...
    def delete_all(self, user_id:Optional[str]=None, agent_id:Optional[str]=None, run_id:Optional[str]=None):
        ...
        memories = asyncio.get_event_loop().run_until_complete(self.vector_store.list(filters=filters))
    ...
    def history(self, memory_id):
        ...
        return asyncio.get_event_loop().run_until_complete(self.db.get_history(memory_id))
    ...
    def _create_memory(self, data, existing_embeddings, metadata=None):
        try:
            ...
            # 确保 MemoryItem 包含 id 字段
            memory_item_dict = MemoryItem(
                id=memory_id,  # 显式传入 id
                memory=data, 
                metadata=metadata
            )
            
            asyncio.get_event_loop().run_until_complete(self.vector_store.insert(
                vectors=[embeddings],
                payloads=[memory_item_dict],
                ids=[str(memory_id)],
            ))
    ...
    def _update_memory(self, memory_id, data, existing_embeddings, metadata=None):
        try:
            ...
            try:
                existing_memory = asyncio.get_event_loop().run_until_complete(self.vector_store.get(vector_id=memory_id))
            except Exception:
            ...
            asyncio.get_event_loop().run_until_complete(self.vector_store.update(
                vector_id=memory_id, vector=embeddings, payload=memory_item_dict
            ))
    ...
    def _delete_memory(self, memory_id):
        ...
        asyncio.get_event_loop().run_until_complete(self.vector_store.delete(vector_id=memory_id))
        self.db.add_history(
            memory_id,
            prev_value,
            None,
    ...
    def reset(self):
        ...
        if hasattr(self.vector_store, "reset"):
            self.vector_store = asyncio.get_event_loop().run_until_complete(VectorStoreFactory.reset(self.vector_store))
        else:
            logger.warning("Vector store does not support reset. Skipping.")
            asyncio.get_event_loop().run_until_complete(self.vector_store.delete_col())
            self.vector_store = VectorStoreFactory.create(
                self.config.vector_store.provider, self.config.vector_store.config
            )
...
class AsyncMemory(MemoryBase):
    ...
    async def _add_to_vector_store(
        ...
        async def process_fact_for_search(new_mem_content):
            ...
            # 直接调用异步的 search 方法，不使用 asyncio.to_thread
            try:
                # 检查 search 方法是否为异步方法
                if asyncio.iscoroutinefunction(self.vector_store.search):
                    existing_mems = await self.vector_store.search(
                    ...
                    # 如果是同步方法，继续使用 asyncio.to_thread
                ...
                # 记录结果类型以便调试
                logging.debug(f"search 返回类型: {type(existing_mems)}, 内容: {existing_mems}")
                
                # 检查 existing_mems 是否为协程对象，如果是则等待它
                if hasattr(existing_mems, '__await__'):
                    logging.warning("search 返回了协程对象，自动等待结果")
                    existing_mems = await existing_mems
                
                return [{"id": mem.id, "text": mem.payload["memory"]} for mem in existing_mems]
            except Exception as e:
                logging.error(f"search 操作失败: {e}")
                import traceback
                traceback.print_exc()
                return []
    ...
    async def _get_all_from_vector_store(self, filters, limit):
        memories_result = await self.vector_store.list(filters=filters, limit=limit)
        actual_memories = memories_result[0] if isinstance(memories_result, tuple) and len(memories_result) > 0 else memories_result
    ...
    async def delete_all(self, user_id=None, agent_id=None, run_id=None):
        ...
        memories = await self.vector_store.list(filters=filters)
    ...
    async def _update_memory(self, memory_id, data, existing_embeddings, metadata=None):
        try:
            ...
            try:
                existing_memory = await self.vector_store.get(vector_id=memory_id)
            except Exception:
                logger.error(f"Error getting memory with ID {memory_id} during update.")
                raise ValueError(f"Error getting memory with ID {memory_id}. Please provide a valid 'memory_id'")
    ...
    async def reset(self):
        ...
        """
        logger.warning("Resetting all memories")
        await self.vector_store.delete_col()

        gc.collect()

        if hasattr(self.vector_store, "client") and hasattr(self.vector_store.client, "close"):
            # 这里 client.close 可能不是异步方法，所以保留 asyncio.to_thread
